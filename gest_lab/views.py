...
def procesar(request):
    ...
    if request.method == 'GET':
        ...
        if t_ex == '':
            l_examenes = examen.objects.values('n_orden','id_cliente__cedula','id_analisis__nombre').filter(id_cliente__cedula__startswith=ced,n_orden__endswith=n_ord).annotate(dcount=Count('n_orden')).order_by()
        else:
            l_examenes = examen.objects.values('n_orden','id_cliente__cedula','id_analisis__nombre').filter(id_cliente__cedula__startswith=ced,n_orden__endswith=n_ord,id_analisis=t_ex).annotate(dcount=Count('n_orden')).order_by()
        # print(l_examenes.query)
    return render(request, 'gest_lab/procesar.html',{'dir_ex':'/procesar', 'analisis':l_analisis, 'l_examenes':l_examenes, 'query':l_examenes.query})
